...
var Intense = function () {
    ...
    function i(e) {
        var t;
        if (e.length) for (t = 0; t < e.length; t++) o(e[t]); else o(e)
    }

    function o(e) {
        (e.getAttribute("data-image") || e.src) && e.addEventListener("click", function () {
            p(this)
        }, !1)
    }
    ...
    function a() {
        cancelRequestAnimFrame(y)
    }

    function d() {
        y = requestAnimFrame(d), b()
    }
    ...
    function w(e, n) {
        ...
        Z = document.createElement("figure"), Z.appendChild(E), t(Z, i);
        ...
        if (t(a, r), e) {
            ...
            t(c, d), c.innerHTML = e, a.appendChild(c)
        }
        if (n) {
            var u = {
                margin: "0px",
            ...
            t(w, u), w.innerHTML = n, a.appendChild(w)
        }
        Z.appendChild(a), s(), M.xCurr = M.xDest = window.innerWidth / 2, M.yCurr = M.yDest = window.innerHeight / 2, document.body.appendChild(Z), setTimeout(function () {
            Z.style.opacity = "1"
        }, 10)
    }

    function m() {
        u(), f(), document.body.removeChild(Z)
    }

    function s() {
        var e = n(C);
        E.width = e.w, E.height = e.h, j = e.fit, F = {w: E.width, h: E.height}, S = {
            w: window.innerWidth,
            h: window.innerHeight
        }, k = {x: S.w - F.w, y: S.h - F.h}
    }

    function p(e) {
        var t = e.getAttribute("data-image") || e.src, n = e.getAttribute("data-title"),
            i = e.getAttribute("data-caption"), o = new Image;
        o.onload = function () {
            C = {w: o.width, h: o.height}, E = this, w(n, i), c(), l(), d()
        }, o.src = t
    }

    function l() {
        Z.addEventListener("mousemove", x, !1), Z.addEventListener("touchmove", h, !1), window.addEventListener("resize", s, !1), window.addEventListener("keyup", v, !1), E.addEventListener("click", m, !1)
    }

    function f() {
        Z.removeEventListener("mousemove", x, !1), Z.removeEventListener("touchmove", h, !1), window.removeEventListener("resize", s, !1), window.removeEventListener("keyup", v, !1), E.removeEventListener("click", m, !1)
    }

    function x(e) {
        $(".video_div").show();
        M.xDest = e.clientX, M.yDest = e.clientY
    }

    function h(e) {
        e.preventDefault(), M.xDest = e.touches[0].clientX, M.yDest = e.touches[0].clientY
    }

    function v(e) {
        e.preventDefault(), e.keyCode === D && m()
    }

    function b() {
        if (M.xCurr += .05 * (M.xDest - M.xCurr), M.yCurr += .05 * (M.yDest - M.yCurr), j === !0) {
            if (R += M.xCurr - R, M.xCurr !== A) {
                var e = parseFloat(R / S.w);
                e = k.x * e, E.style.webkitTransform = "translate3d(" + e + "px, 0px, 0px)", E.style.MozTransform = "translate3d(" + e + "px, 0px, 0px)", E.style.msTransform = "translate3d(" + e + "px, 0px, 0px)", A = M.xCurr
            }
        } else if (j === !1 && (R += M.yCurr - R, M.yCurr !== A)) {
            var e = parseFloat(R / S.h);
            e = k.y * e, E.style.webkitTransform = "translate3d( 0px, " + e + "px, 0px)", E.style.MozTransform = "translate3d( 0px, " + e + "px, 0px)", E.style.msTransform = "translate3d( 0px, " + e + "px, 0px)", A = M.yCurr
        }
    }
    ...
    var y, A, C, E, Z, I, D = 27, M = {xCurr: 0, yCurr: 0, xDest: 0, yDest: 0}, j = !0, R = 0, F = {w: 0, h: 0},
        S = {w: 0, h: 0}, k = {x: 0, y: 0};
    return e(g, {resize: s, start: r, stop: a})
}();